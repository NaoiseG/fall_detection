...
The script writes:
  - ONNX files for the requested stages
...
def parse_args() -> argparse.Namespace:
    ...
    ap.add_argument(
        "--detector-only",
        action="store_true",
        help="Export only the RT-DETR detector stage and omit ViTPose pose export.",
    )
...
def write_manifest_files(
    ...
    pose_info: Optional[StageExportInfo],
) -> tuple[Path, Path]:
    ...
    env_lines = [
        ...
        f"POSE_PRESENT={1 if pose_info is not None else 0}",
    ]
    if pose_info is not None:
        env_lines.extend(
            [
...
def main() -> int:
    ...
    pose_model: Optional[VitPoseForPoseEstimation] = None
    ...
    pose_info: Optional[StageExportInfo] = None
    if pose_model is not None:
    ...
    if pose_info is not None:
        ...
        print("Skipped pose ONNX export (--detector-only).")
