...
# ========================================================
from ultralytics.nn.modules.block import Bottleneck, Attention, PSABlock, C2PSA, DWConv
                ...
                if self.sr:
                    ...
                    for k, m in self.model.named_modules():
                        if isinstance(m, Bottleneck):
                            if m.add: # 虽然在v11中是C3k2模块, 但实际上上和v8中的C2f模块对忽略剪枝的bn层的处理是一样的
                                ignore_bn_list.append(k.rsplit(".", 2)[0] + ".cv1.bn")  
                                ignore_bn_list.append(k + '.cv2.bn')
                        if isinstance(m, Attention):
                            ignore_bn_list.append(k + ".qkv.bn")                        # Attention模块中的qkv中的bn层
                            ignore_bn_list.append(k + ".pe.bn")                         # Attention模块中的pe分支的卷积层的bn层
                            ignore_bn_list.append(k + ".proj.bn")                       # Attention模块中的proj分支的卷积层的bn层
                        if isinstance(m, PSABlock):
                            ignore_bn_list.append(k + ".ffn.1.bn")                      # Attention模块中的ffn分支的第二个卷积层的bn层
                        if isinstance(m, C2PSA):
                            ignore_bn_list.append(k + ".cv1.bn")
                        if isinstance(m, Detect):                                     # Detect模块右侧分支的中两个深度可分离卷积之间的卷积
                            for i in range(3):
                                ignore_bn_list.append(k + f".cv3.{i}.0.1.bn")
                                ignore_bn_list.append(k + f".cv3.{i}.1.1.bn")
                        if isinstance(m, DWConv):
                            ignore_bn_list.append(k + ".bn")
                        if isinstance(m, nn.BatchNorm2d) and (k not in ignore_bn_list):
                            m.weight.grad.data.add_(srtmp * torch.sign(m.weight.data))  # L1
                for ignore_bn in ignore_bn_list:
                    assert ignore_bn in ignore_bn_list, f"There are some errors in your code, {ignore_bn} not in ignore_bn_list"
                # ============================= sparsity training ========================== 
