...
class GRUBaseline(nn.Module):
    """
    Minimal GRU baseline:
      (B, T, F)
        -> GRU over time
        -> take last hidden state (or mean pool over time)
    ...
    def __init__(
        ...
        # Note: dropout is only applied between GRU layers when num_layers > 1
        self.gru = nn.GRU(
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        """
        out, _ = self.gru(x)  # out: (B, T, H*dirs)
