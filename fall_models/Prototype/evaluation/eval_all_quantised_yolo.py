#!/usr/bin/env python3
"""Resumable UP-Fall evaluation sweep over keypoint pose variants."""
...
WINDOW_STRIDE = 32
LABEL_MODE = "center"
MASTER_JSON_NAME = "results_master.json"
...
class RunSpec:
    ...
    @property
    def run_id(self) -> str:
        return f"{self.classifier_model}__{self.pose_model_size}__{self.precision}"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Optional subset of classifier models: cnnlstm stgcn MotionBERT",
    )
    parser.add_argument(
        "--only-pose-sizes",
        nargs="+",
    ...
    parser.add_argument(
        "--force-rerun",
        action="store_true",
        help="Rerun selected runs even if they already have a valid completed status.",
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Print the selected run plan and commands without launching subprocesses or writing outputs.",
    )
    parser.add_argument(
        "--keypoints-root",
        type=Path,
    ...
    parser.add_argument(
        ...
        help=f"Classifier checkpoints root (default: {DEFAULT_CLASSIFICATION_ROOT.as_posix()})",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
        default=DEFAULT_OUTPUT_ROOT,
        help=f"Shared evaluation output root (default: {DEFAULT_OUTPUT_ROOT.as_posix()})",
    )
    parser.add_argument(
        "--motionbert-pkl-root",
        type=Path,
...
def get_run_dir(paths: SweepPaths, classifier_model: str, pose_size: str, precision: str) -> Path:
    return paths.output_root / classifier_model / pose_size / precision


def get_motionbert_pkl_paths(paths: SweepPaths, pose_size: str, precision: str) -> Tuple[Path, Path]:
    out_dir = paths.motionbert_pkl_root / pose_size / precision
    stem = (
        f"upfall_motionbert_{pose_size}_{precision}_cam_{'_'.join(str(c) for c in CAMERAS)}"
...
def execute_run(paths: SweepPaths, run: RunSpec) -> Dict[str, Any]:
    ...
    try:
        with run.stdout_log_path.open("w", encoding="utf-8") as stdout_handle, run.stderr_log_path.open(
            ...
            if run.classifier_model == "MotionBERT":
                if valid_motionbert_pkl(run.motionbert_pkl_path, run.motionbert_label_map_path):
...
def main() -> int:
    ...
    all_runs = build_run_matrix(paths)
    selected_runs = [
        run
        for run in all_runs
        if is_selected_run(run, selected_models, selected_pose_sizes, selected_precisions)
    ]
    ...
    if not args.dry_run:
        ...
        write_master_json(master_json_path, paths, all_runs, master_entries)
