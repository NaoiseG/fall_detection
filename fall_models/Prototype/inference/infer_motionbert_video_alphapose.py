...
def build_arg_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="MotionBERT wrapper on shared benchmark core using AlphaPose keypoints.")
    ...
    ap.add_argument(
        ...
        type=float,
        default=0.10,
        help="AlphaPose detector confidence threshold. --conf-thres kept for CLI compatibility.",
    )
    ap.add_argument("--alphapose-nms", "--nms-thres", dest="alphapose_nms", type=float, default=0.60)
    ap.add_argument("--alphapose-flip", type=int, default=0, help="Enable test-time flip augmentation in AlphaPose (0/1).")
    ...
    ap.add_argument("--out-pkl", type=str, default="outputs/motionbert_video_alphapose.pkl")
    ap.add_argument("--out-csv", type=str, default="outputs/motionbert_video_alphapose_preds.csv")
...
def main() -> int:
    ...
    if profile_enabled:
        profile_out_dir = pick_profile_out_dir(
            ...
            save_path=save_path,
            model_tag="motionbert_alphapose",
    ...
    run_cfg = BenchmarkRunConfig(
        ...
        benchmark_name="MotionBERT Inference (AlphaPose)",
    ...
    if out_pkl is not None:
        ...
        if payload_ready:
            ...
            export_cfg = BenchmarkRunConfig(
                video_path=video_path,
                benchmark_name="MotionBERT Compatibility Export (AlphaPose)",
