...
from dataset_helpers.get_keypoints_files_ViTpose import DEFAULT_DETECTOR_MODEL, DEFAULT_POSE_MODEL
...
from inference.pose_pipeline_vitpose import VitPosePipeline, VitPosePipelineConfig
...
def _resolve_model_source(path_arg: str) -> str:
    ...
    repo_rel = (_REPO_ROOT / raw).expanduser()
    if repo_rel.exists():
        return str(repo_rel)

    if p.suffix.lower() in {".engine", ".pt", ".pth", ".onnx", ".bin", ".ckpt"} or p.is_absolute():
        raise FileNotFoundError(f"Model source not found: {path_arg}")
...
def _build_motionbert_pose_pipeline(
    *,
    ...
    return VitPosePipeline(
        VitPosePipelineConfig(
            detector_model=_resolve_model_source(args.vitpose_detector_model),
            detector_processor=_resolve_model_source(args.vitpose_detector_processor) if args.vitpose_detector_processor else None,
            pose_model=_resolve_model_source(args.vitpose_pose_model),
            pose_processor=_resolve_model_source(args.vitpose_pose_processor) if args.vitpose_pose_processor else None,
            device=str(device),
            vitpose_conf=float(args.vitpose_conf),
            vitpose_pose_threshold=float(args.vitpose_pose_threshold),
...
def build_arg_parser() -> argparse.ArgumentParser:
    ap = argparse.ArgumentParser(description="MotionBERT wrapper on shared benchmark core using ViTPose keypoints.")
    ...
    ap.add_argument("--vitpose-detector-model", type=str, default=DEFAULT_DETECTOR_MODEL)
    ap.add_argument("--vitpose-detector-processor", type=str, default=None)
    ap.add_argument("--vitpose-pose-model", type=str, default=DEFAULT_POSE_MODEL)
    ap.add_argument("--vitpose-pose-processor", type=str, default=None)
    ap.add_argument(
        "--vitpose-conf",
        "--conf-thres",
        dest="vitpose_conf",
        type=float,
        default=0.25,
        help="ViTPose detector confidence threshold. --conf-thres kept for CLI compatibility.",
    )
    ap.add_argument("--vitpose-pose-threshold", type=float, default=0.30)
    ap.add_argument(
        "--max-det",
        type=int,
        default=10,
        help="ViTPose detector candidate cap. <=0 keeps all candidates.",
    )
    ...
    ap.add_argument("--out-pkl", type=str, default="outputs/motionbert_video_vitpose.pkl")
    ap.add_argument("--out-csv", type=str, default="outputs/motionbert_video_vitpose_preds.csv")
...
def main() -> int:
    ...
    if profile_enabled:
        profile_out_dir = pick_profile_out_dir(
            ...
            save_path=save_path,
            model_tag="motionbert_vitpose",
            yolo_weights_path=pose_pipeline.pose_model_path,
        )
    ...
    run_cfg = BenchmarkRunConfig(
        ...
        benchmark_name="MotionBERT Inference (ViTPose)",
    ...
    if out_pkl is not None:
        ...
        if payload_ready:
            ...
            export_cfg = BenchmarkRunConfig(
                video_path=video_path,
                benchmark_name="MotionBERT Compatibility Export (ViTPose)",
