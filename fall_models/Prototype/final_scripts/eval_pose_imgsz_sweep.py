...
def suggest_model_path(model_path: Path) -> Path | None:
    path_text = str(model_path)
    typo_fixes = {
        "yolo1xn-pose": "yolo11x-pose",
    }
    for typo, fix in typo_fixes.items():
        if typo in path_text:
            return Path(path_text.replace(typo, fix))

    if re.fullmatch(r"yolo11[nsmlx]-pose\.pt", model_path.name):
        expected_parent = model_path.stem
        if model_path.parent.name and model_path.parent.name != expected_parent:
            return model_path.parent.parent / expected_parent / model_path.name

    return None


def missing_weights_error(model_path: Path) -> str:
    message = f"FileNotFoundError: weights not found: {model_path}"
    suggestion = suggest_model_path(model_path)
...
def run_single_eval(YOLO, args: argparse.Namespace, model_path: Path, imgsz: int) -> Dict[str, Any]:
    ...
    if not model_path.exists():
        row["status"] = "failed"
        row["error"] = missing_weights_error(model_path)
        return row
