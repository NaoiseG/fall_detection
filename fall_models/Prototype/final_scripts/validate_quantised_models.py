...
DEFAULT_CSV = "engine_coco_pose_val_results.csv"
DEFAULT_JSON = "engine_coco_pose_val_results.json"


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Validate TensorRT YOLO pose .engine models on the COCO-Pose validation split."
    )
    parser.add_argument(
        "--models-root",
        type=Path,
        default=Path("models/ultralytics/yolo11"),
        help="Root directory to recursively search for .engine files.",
    )
    parser.add_argument(
        "--data-yaml",
        type=Path,
        default=Path("../Datasets/coco-pose/coco-pose.yaml"),
        help="Path to the existing COCO-Pose dataset YAML.",
    )
    parser.add_argument(
        "--split",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Validation batch size. Default 1 for TensorRT/Jetson safety.",
    )
    parser.add_argument(
        "--device",
        type=str,
        default="0",
        help="Ultralytics device argument. On Jetson, '0' is typical for GPU.",
    )
    parser.add_argument(
        "--workers",
        type=int,
        default=0,
        help="Dataloader workers. Default 0 is conservative on Jetson.",
    )
    parser.add_argument(
        "--conf",
        type=float,
    ...
    parser.add_argument(
        ...
        help="Combined CSV summary output path.",
    )
    parser.add_argument(
        "--json-out",
        type=Path,
        default=Path(DEFAULT_JSON),
        help="Combined JSON summary output path.",
    )
    parser.add_argument(
        "--force",
        action="store_true",
        help="Ignore prior results and re-run every discovered engine.",
    )
    parser.add_argument(
        "--rerun-failed",
        action="store_true",
    ...
    parser.add_argument(
        "--project",
        type=Path,
        default=Path("runs/engine_pose_val"),
        help="Ultralytics project directory for per-engine validation logs.",
    )
    parser.add_argument(
        "--name-prefix",
        type=str,
...
def infer_engine_metadata(engine_path: Path) -> dict[str, str]:
    name = engine_path.name
    match = ENGINE_RE.match(name)
    if match:
        arch = match.group("arch")
        version = match.group("version") or "engine"
    else:
        stem = engine_path.stem
        parts = stem.split("_")
        arch = parts[0] if parts else engine_path.parent.name
...
def discover_engines(models_root: Path) -> list[Path]:
    root = models_root.expanduser().resolve()
    if not root.exists():
        raise FileNotFoundError(f"Models root does not exist: {root}")
    engines = sorted(p for p in root.rglob("*.engine") if p.is_file())
    return engines
...
def resolve_dataset_root(yaml_path: Path, data: dict[str, Any]) -> Path:
    ...
    # Common case: YAML is already inside dataset root but still says `path: coco-pose`
...
def resolve_split_entry(entry: Any, dataset_root: Path, yaml_dir: Path) -> Any:
    ...
    if isinstance(entry, list):
        return [resolve_split_entry(x, dataset_root, yaml_dir) for x in entry]
    if entry is None:
        return None
    return entry
...
def create_val_only_yaml(source_yaml: Path, split: str) -> tuple[Path, dict[str, Any]]:
    ...
    temp_yaml["train"] = resolved_split  # mirror target split so missing train data does not block val-only use
    ...
    # Avoid automatic download side effects.
...
def evaluate_engine(
    ...
    run_name = f"{args.name_prefix}{Path(engine_path).stem}"
    result: dict[str, Any] = {
        **meta,
        "status": "failed",
        "pose_map50_95": None,
...
def write_csv(path: Path, rows: list[dict[str, Any]]) -> None:
    ...
    with path.expanduser().resolve().open("w", encoding="utf-8", newline="") as f:
...
def write_json(path: Path, rows: list[dict[str, Any]], dataset_info: dict[str, Any]) -> None:
    ensure_parent_dir(path)
    payload = {
...
def print_table(rows: list[dict[str, Any]], title: str) -> None:
    ...
    for header, key in columns:
        widths[key] = max(len(header), *(len(r[key]) for r in display_rows))
...
def main() -> int:
    ...
    try:
        ...
        engines = discover_engines(models_root)
        if not engines:
            print(f"No .engine files found under: {models_root}")
            return 1

        existing = {} if args.force else load_existing_results(csv_out, json_out)

        discovered_meta = [infer_engine_metadata(p) for p in engines]
        ...
        print(f"Discovered {len(engines)} .engine files under {models_root}")
        ...
        write_json(json_out, final_rows, dataset_info)

        new_successes = [r for r in run_results if r.get("status") == "success"]
        new_failures = [r for r in run_results if r.get("status") == "failed"]
