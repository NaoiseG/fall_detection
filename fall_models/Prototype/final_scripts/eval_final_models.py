#!/usr/bin/env python3
"""Evaluate all final temporal classifiers across all discovered keypoint variants."""
...
CLASSIFIER_MODELS: Tuple[str, ...] = ("tcn", "cnnlstm", "paper_stgcn", "MotionBERT")
...
SOURCE_TYPE_ORDER: Tuple[str, ...] = ("standard", "pruned")
PRECISION_ORDER: Tuple[str, ...] = ("base", "fp16", "fp32", "int8")

YOLO_POSE_PATTERN = re.compile(r"^yolo11[nsmlx]$")
PRUNE_VARIANT_PATTERN = re.compile(r"^pruned_(\d+)$")

ALPHAPOSE_PRECISION_MAP: Dict[str, str] = {
    "base": "base",
    "fp16_fp16": "fp16",
    "fp32_fp32": "fp32",
    "int8_fp16": "int8",
}
VITPOSE_PRECISION_MAP: Dict[str, str] = {
    "base": "base",
    "fp16": "fp16",
    "fp32_fp32": "fp32",
}

DEFAULT_KEYPOINTS_ROOT = Path("/home/people/21376026/scratch/keypoints")
DEFAULT_CLASSIFICATION_ROOT = Path("/home/people/21376026/scratch/final_classification_models")
DEFAULT_OUTPUT_ROOT = Path("/home/people/21376026/scratch/final_results/final_evaluations")
...
class SweepPaths:
    ...
    keypoints_root: Path
    standard_yolo_root: Path
    standard_alphapose_root: Path
    standard_vitpose_root: Path
    pruned_root: Path
...
class KeypointVariant:
    ...
    npz_root: Path
    prune_variant: Optional[str] = None

    @property
    def variant_id(self) -> str:
        if self.prune_variant is None:
            return f"{self.source_type}__{self.pose_model_size}__{self.precision}"
        return f"{self.source_type}__{self.pose_model_size}__{self.prune_variant}__{self.precision}"

    @property
    def keypoints_tag(self) -> str:
        if self.prune_variant is None:
            return f"{self.pose_model_size}/{self.precision}"
        return f"{self.pose_model_size}/{self.prune_variant}/{self.precision}"
...
class RunSpec:
    ...
    def run_id(self) -> str:
        if self.prune_variant is None:
            return f"{self.classifier_model}__{self.source_type}__{self.pose_model_size}__{self.precision}"
        return (
            f"{self.classifier_model}__{self.source_type}__{self.pose_model_size}"
...
def source_type_sort_key(source_type: str) -> Tuple[int, str]:
    index = SOURCE_TYPE_ORDER.index(source_type) if source_type in SOURCE_TYPE_ORDER else 999
    return index, source_type
...
def precision_sort_key(precision: str) -> Tuple[int, str]:
    index = PRECISION_ORDER.index(precision) if precision in PRECISION_ORDER else 999
    return index, precision


def prune_sort_key(prune_variant: Optional[str]) -> Tuple[int, str]:
    if prune_variant is None:
        return -1, ""
...
def normalize_standard_precision(pose_model_size: str, raw_dir_name: str) -> str:
    cleaned = str(raw_dir_name).strip()
    if pose_model_size == "alphapose":
        return ALPHAPOSE_PRECISION_MAP.get(cleaned, cleaned.lower())
    if pose_model_size == "vitpose":
        return VITPOSE_PRECISION_MAP.get(cleaned, cleaned.lower())
    return cleaned.lower()


def discover_standard_variants(paths: SweepPaths) -> Tuple[List[KeypointVariant], List[str]]:
    issues: List[str] = []
    variants: List[KeypointVariant] = []

    if not paths.standard_yolo_root.is_dir():
        issues.append(
            f"Standard YOLO keypoints root not found: {paths.standard_yolo_root.as_posix()}"
        )
    else:
        for pose_dir in sorted(paths.standard_yolo_root.iterdir(), key=lambda item: item.name):
            if not pose_dir.is_dir():
                continue
            pose_name = pose_dir.name
            if YOLO_POSE_PATTERN.fullmatch(pose_name) is None:
                issues.append(f"Skipping unrecognized YOLO pose directory: {pose_dir.as_posix()}")
                continue
            for precision_dir in sorted(pose_dir.iterdir(), key=lambda item: item.name):
                ...
                variants.append(
                    KeypointVariant(
                        ...
                        precision=normalize_standard_precision(pose_name, precision_dir.name),
                        variant_dir_name=precision_dir.name,
                        npz_root=precision_dir,
                    )
                )

    for pose_name, root in (
        ("alphapose", paths.standard_alphapose_root),
        ("vitpose", paths.standard_vitpose_root),
    ):
        if not root.is_dir():
            issues.append(f"{pose_name} keypoints root not found: {root.as_posix()}")
            continue
        for precision_dir in sorted(root.iterdir(), key=lambda item: item.name):
            if not precision_dir.is_dir():
                continue
            variants.append(
                KeypointVariant(
    ...
    variants.sort(
        key=lambda item: (
            source_type_sort_key(item.source_type),
            pose_sort_key(item.pose_model_size),
            prune_sort_key(item.prune_variant),
            precision_sort_key(item.precision),
            item.variant_dir_name,
        )
    )
    return variants, issues


def discover_pruned_variants(paths: SweepPaths) -> Tuple[List[KeypointVariant], List[str]]:
    issues: List[str] = []
    variants: List[KeypointVariant] = []

    if not paths.pruned_root.is_dir():
        issues.append(f"Pruned keypoints root not found: {paths.pruned_root.as_posix()}")
        return variants, issues

    for pose_dir in sorted(paths.pruned_root.iterdir(), key=lambda item: item.name):
        if not pose_dir.is_dir():
            continue
        pose_name = pose_dir.name
        if YOLO_POSE_PATTERN.fullmatch(pose_name) is None:
            issues.append(f"Skipping unrecognized pruned pose directory: {pose_dir.as_posix()}")
            continue
        for prune_dir in sorted(pose_dir.iterdir(), key=lambda item: item.name):
            if not prune_dir.is_dir():
                continue
            prune_name = prune_dir.name
            if PRUNE_VARIANT_PATTERN.fullmatch(prune_name) is None:
...
def discover_all_variants(paths: SweepPaths) -> Tuple[List[KeypointVariant], List[str]]:
    standard_variants, standard_issues = discover_standard_variants(paths)
    pruned_variants, pruned_issues = discover_pruned_variants(paths)
    variants = standard_variants + pruned_variants
    variants.sort(
        key=lambda item: (
            source_type_sort_key(item.source_type),
            pose_sort_key(item.pose_model_size),
    ...
    return variants, standard_issues + pruned_issues
...
def get_run_dir(paths: SweepPaths, classifier_model: str, variant: KeypointVariant) -> Path:
    base = paths.output_root / classifier_model / variant.pose_model_size
    if variant.prune_variant is not None:
        base = base / variant.prune_variant
    return base / variant.precision


def get_motionbert_pkl_paths(paths: SweepPaths, variant: KeypointVariant) -> Tuple[Path, Path]:
    out_dir = paths.motionbert_pkl_root / variant.pose_model_size
    if variant.prune_variant is not None:
        out_dir = out_dir / variant.prune_variant
    out_dir = out_dir / variant.precision
    stem_parts = [
        "upfall_motionbert",
        variant.pose_model_size,
    ]
    if variant.prune_variant is not None:
        stem_parts.append(variant.prune_variant)
    stem_parts.extend(
        [
            variant.precision,
            f"cam_{'_'.join(str(c) for c in CAMERAS)}",
            f"train_{TRAIN_SUBJECTS.replace('-', '_')}",
            f"val_{VAL_SUBJECTS.replace('-', '_')}",
            f"label_{LABEL_MODE}",
            f"win_{WINDOW_T}",
            f"step_{WINDOW_STRIDE}",
        ]
    )
        ...
        for classifier_model in CLASSIFIER_MODELS:
            ...
            runs.append(
                RunSpec(
                    ...
                    variant_dir_name=variant.variant_dir_name,
...
def is_selected_run(
    ...
    *,
    ...
    selected_source_types: Optional[Sequence[str]],
    selected_precisions: Optional[Sequence[str]],
    selected_prune_variants: Optional[Sequence[str]],
) -> bool:
    ...
    if selected_pose_sizes is not None and run.pose_model_size not in selected_pose_sizes:
        return False
    if selected_source_types is not None and run.source_type not in selected_source_types:
        return False
    if selected_precisions is not None and run.precision not in selected_precisions:
        return False
    if selected_prune_variants is not None:
        if run.prune_variant is None or run.prune_variant not in selected_prune_variants:
            return False
    return True
...
def base_summary_entry(run: RunSpec) -> Dict[str, Any]:
    return {
        ...
        "keypoint_precision": run.precision,
...
def parse_overall_metrics_table(report_html: Path) -> "pd.DataFrame":
    ...
    for table in tables:
        ...
        if {"accuracy", "recall", "precision"}.issubset(set(norm.columns)):
            return norm
    ...
    return normalize_metrics(
        {
            ...
            "precision": normalize_metric_0_to_100(overall_row.get("precision")),
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate all final classifiers across all discovered standard and pruned keypoint variants."
    )
    parser.add_argument("--only-models", nargs="+", default=None)
    parser.add_argument("--only-pose-sizes", nargs="+", default=None)
    parser.add_argument("--only-source-types", nargs="+", default=None, help="Subset of: standard pruned")
...
def build_paths(args: argparse.Namespace) -> SweepPaths:
    ...
    return SweepPaths(
        ...
        standard_yolo_root=(keypoints_root / "UPFall_keypoints").expanduser(),
        standard_alphapose_root=(keypoints_root / "UPFall_keypoints_alpha").expanduser(),
        standard_vitpose_root=(keypoints_root / "UPFall_keypoints_vitpose").expanduser(),
        pruned_root=(keypoints_root / "pruned_keypoints" / "full_pruned").expanduser(),
...
def main() -> int:
    ...
    variants, discovery_issues = discover_all_variants(paths)
    if not variants:
        print("No keypoint variants were discovered.", file=sys.stderr)
    ...
    discovered_precisions = tuple(sorted({variant.precision for variant in variants}, key=precision_sort_key))
    discovered_prune_variants = tuple(
        sorted({variant.prune_variant for variant in variants if variant.prune_variant is not None}, key=prune_sort_key)
    )
    ...
    selected_source_types = normalize_filter_tokens(
        args.only_source_types,
        allowed=SOURCE_TYPE_ORDER,
        kind="source type",
        transform=str.lower,
    )
    ...
    selected_runs = [
        ...
        if is_selected_run(
            ...
            selected_prune_variants=selected_prune_variants,
        )
    ]
        ...
        if status == "completed":
            ...
            if isinstance(metrics, dict):
                print(
                    ...
                    f"prec={metrics.get('precision')}, macro_f1={metrics.get('macro_f1')}, "
                    f"fall_f1={metrics.get('fall_f1')})"
                )
            else:
                print("  completed")
